<?php
/**
 * qreate\qreate\Styles\Component class
 *
 * @package qreate
 */

namespace qreate\qreate\Styles;

use qreate\qreate\Component_Interface;
use qreate\qreate\Templating_Component_Interface;
use function qreate\qreate\qreate;
...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function action_enqueue_styles() {
		...
		if ( ! empty( $google_fonts_url ) ) {
			wp_enqueue_style( 'qreate-fonts', $google_fonts_url, array(), null ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}
		...
		foreach ( $css_files as $handle => $data ) {
			...
			$version = qreate()->get_asset_version( $css_dir . $data['file'] );
	...
	public function filter_resource_hints( array $urls, string $relation_type ) : array {
		if ( 'preconnect' === $relation_type && wp_style_is( 'qreate-fonts', 'queue' ) ) {
			$urls[] = array(
		...
		$handles   = array_filter(
			...
			function( $handle ) use ( $css_files ) {
				...
				if ( ! $is_valid ) {
					...
					_doing_it_wrong( __CLASS__ . '::print_styles()', esc_html( sprintf( __( 'Invalid theme stylesheet handle: %s', 'qreate' ), $handle ) ), 'Qreate 2.0.0' );
				}
				return $is_valid;
			}
		);
	 ...
	 * {@see 'qreate_preloading_styles_enabled'} filter can be used to tweak the return value.
	 *
	...
	protected function preloading_styles_enabled() {
		$preloading_styles_enabled = ! qreate()->is_amp();
		 ...
		 */
		return apply_filters( 'qreate_preloading_styles_enabled', $preloading_styles_enabled );
	}
	...
	protected function get_css_files() : array {
		...
		$css_files = array(
			...
			'qreate-sidebar'    => array(
				'file'             => 'sidebar.min.css',
				'preload_callback' => function() {
					return qreate()->is_primary_sidebar_active();
				},
				'global' => true,
			),
			'qreate-widgets'    => array(
		 ...
		 */
		$css_files = apply_filters( 'qreate_css_files', $css_files );
	...
	protected function get_google_fonts() : array {
		...
		$google_fonts = array(
			...
			'Raleway'     => array( '400', '400i','500', '500i','600', '600i','700', '700i'),
		);
		 ...
		 */
		$this->google_fonts = (array) apply_filters( 'qreate_google_fonts', $google_fonts );
