...
namespace qreate\qreate\Footer;
...
class Component implements Component_Interface, Templating_Component_Interface
{
	...
	public function register_footers($footer_option)
	{
		...
		$theme_option = get_option('qreate-options');

		$default = [
			'1' => esc_html__('text-left', 'qreate'),
			'2' => esc_html__('text-right', 'qreate'),
			'3' => esc_html__('text-center', 'qreate'),
		];
		...
		foreach ($footer_option as $key => $item) {
			...
			register_sidebar(
				array(
					'name'          => esc_html__('Footer Area ' . $key, 'qreate'),
					'class'         => 'nav-list',
					'id'            => 'footer_' . ($key) . '_sidebar',
					'before_widget' => '<div class="widget %2$s ' . esc_attr($footer) . '">',
					'after_widget'  => '</div>',
					'before_title'  => '<h4 class="footer-title mt-0"><span class="title">',
					'after_title'   => '</span><span class="line wow"></span></h4>',
				)
			);
		}
	}
		...
		if (
			...
			if (function_exists('get_field') && class_exists('ReduxFramework')) {
				...
				if (isset($acf_footer_option) && $acf_footer_option != "default") {
					...
					$options = !empty($qreate_options['qreate_footer_width']) ? $qreate_options['qreate_footer_width'] : '';
				}
				...
				switch ($options) {
					...
					case 3:
						...
						break;
					case 4:
						$data['value'] = ['col-lg-2 col-sm-6 mt-lg-0 mt-4', 'col-lg-3 col-sm-6 mt-lg-0 mt-4', 'col-lg-3 col-sm-6 mt-lg-0 mt-4', 'col-lg-4 col-sm-6 mt-lg-0 mt-4'];
						break;
					case 5:
				...
				$data['value'] = ['col-lg-4 col-sm-6 mb-4 mb-sm-0', 'col-lg-4 col-sm-6', 'col-lg-4 col-sm-6 mb-4 mb-sm-0'];
			}
		}
		return $data;
	}
