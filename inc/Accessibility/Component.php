...
/**
 * qreate\qreate\Accessibility\Component class
 *
 * @package qreate
 */

namespace qreate\qreate\Accessibility;
...
class Component implements Component_Interface
{
	...
	public function action_enqueue_navigation_script()
	{
		...
		wp_script_add_data('qreate-navigation', 'async', true);
		wp_script_add_data('qreate-navigation', 'precache', true);
		wp_localize_script(
			'qreate-navigation',
			'qreateScreenReaderText',
			array(
				'expand'   => __('Expand child menu', 'qreate'),
				'collapse' => __('Collapse child menu', 'qreate'),
			)
		);
	}
	...
	public function action_print_skip_link_focus_fix()
	{
		...
		wp_register_script('qreate-minified', '', false, true);
		wp_enqueue_script('qreate-minified', '', array(), false, true);
		wp_add_inline_script('qreate-minified', '/(trident|msie)/i.test(navigator.userAgent)&&document.getElementById&&window.addEventListener&&window.addEventListener("hashchange",function(){var t,e=location.hash.substring(1);/^[A-z0-9_-]+$/.test(e)&&(t=document.getElementById(e))&&(/^(?:a|select|input|button|textarea)$/i.test(t.tagName)||(t.tabIndex=-1),t.focus())},!1);');
	}
